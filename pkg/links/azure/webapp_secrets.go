...
func NewAzureWebAppSecretsLink(args map[string]any) *AzureWebAppSecretsLink {
	return &AzureWebAppSecretsLink{
		NativeAzureLink: base.NewNativeAzureLink("webapp-secrets", args),
	}
}
func (l *AzureWebAppSecretsLink) Parameters() []plugin.Parameter {
	return []plugin.Parameter{
		options.AzureSubscription(),
	}
}
func (l *AzureWebAppSecretsLink) Process(ctx context.Context, input any) ([]any, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to parse web app resource ID: %w", err)
	}
	...
	l.Logger().Debug("Getting web app application settings",
		...
		"app_name", appName)
	settings, err := client.ListApplicationSettings(ctx, resourceGroup, appName, nil)
	if err != nil {
	...
	l.Logger().Debug("Getting web app connection strings",
		"resource_group", resourceGroup,
		"app_name", appName)
	connStrings, err := client.ListConnectionStrings(ctx, resourceGroup, appName, nil)
	if err != nil {
	...
	l.Logger().Debug("Getting web app site configuration",
		"resource_group", resourceGroup,
		"app_name", appName)
	config, err := client.GetConfiguration(ctx, resourceGroup, appName, nil)
	if err != nil {
	...
	if l.isFunctionApp(resource) {
		l.Logger().Debug("Getting function app keys",
			"resource_group", resourceGroup,
			"app_name", appName)
		keys, err := client.ListHostKeys(ctx, resourceGroup, appName, nil)
		if err != nil {
