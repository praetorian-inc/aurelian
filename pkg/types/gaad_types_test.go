...
func TestGaadUnmarshal(t *testing.T) {
	t.Run("full GAAD with all detail lists", func(t *testing.T) {
		...
		assert.Equal(t, 1, gaad.Users.Len())
		user, ok := gaad.Users.Get("arn:aws:iam::123456789012:user/admin")
		require.True(t, ok)
		assert.Equal(t, "admin", user.UserName)
		assert.Equal(t, "arn:aws:iam::123456789012:user/admin", user.Arn)
		assert.Equal(t, []string{"Admins"}, user.GroupList)
		require.Len(t, user.Tags, 1)
		assert.Equal(t, "env", user.Tags[0].Key)
		assert.Equal(t, "prod", user.Tags[0].Value)
		require.Len(t, user.AttachedManagedPolicies, 1)
		assert.Equal(t, "AdministratorAccess", user.AttachedManagedPolicies[0].PolicyName)

		assert.Equal(t, 1, gaad.Roles.Len())
		role, ok := gaad.Roles.Get("arn:aws:iam::123456789012:role/LambdaRole")
		require.True(t, ok)
		assert.Equal(t, "LambdaRole", role.RoleName)
		assert.Equal(t, "/service-role/", role.Path)
		require.NotNil(t, role.AssumeRolePolicyDocument.Statement)

		assert.Equal(t, 1, gaad.Groups.Len())
		group, ok := gaad.Groups.Get("arn:aws:iam::123456789012:group/Admins")
		require.True(t, ok)
		assert.Equal(t, "Admins", group.GroupName)

		assert.Equal(t, 0, gaad.Policies.Len())
	})
	...
	t.Run("empty GAAD parses with empty lists", func(t *testing.T) {
		...
		err := json.Unmarshal([]byte(input), &gaad)
		require.NoError(t, err)
		assert.Equal(t, 0, gaad.Users.Len())
		assert.Equal(t, 0, gaad.Roles.Len())
		assert.Equal(t, 0, gaad.Groups.Len())
		assert.Equal(t, 0, gaad.Policies.Len())
	})
}
