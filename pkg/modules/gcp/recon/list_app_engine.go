package recon

import (
	"github.com/praetorian-inc/janus-framework/pkg/chain"
	"github.com/praetorian-inc/janus-framework/pkg/chain/cfg"
	"github.com/praetorian-inc/diocletian/internal/registry"
	"github.com/praetorian-inc/diocletian/pkg/links/gcp/applications"
	"github.com/praetorian-inc/diocletian/pkg/links/gcp/hierarchy"
	"github.com/praetorian-inc/diocletian/pkg/links/options"
	"github.com/praetorian-inc/diocletian/pkg/outputters"
)

func init() {
	registry.Register("gcp", "recon", GcpListAppEngine.Metadata().Properties()["id"].(string), *GcpListAppEngine)
}

var GcpListAppEngine = chain.NewModule(
	cfg.NewMetadata(
		"GCP List App Engine",
		"List all App Engine applications in a GCP project.",
	).WithProperties(map[string]any{
		"id":          "app-engine-list",
		"platform":    "gcp",
		"opsec_level": "moderate",
		"authors":     []string{"Praetorian"},
		"references":  []string{},
	}).WithChainInputParam(options.GcpProject().Name()),
).WithLinks(
	hierarchy.NewGcpProjectInfoLink,
	applications.NewGcpAppEngineApplicationListLink,
).WithOutputters(
	outputters.NewRuntimeJSONOutputter,
).WithInputParam(
	options.GcpProject(),
).WithParams(
	cfg.NewParam[string]("module-name", "name of the module for dynamic file naming"),
).WithConfigs(
	cfg.WithArg("module-name", "app-engine-list"),
)
