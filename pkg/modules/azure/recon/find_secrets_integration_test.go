...
func TestAzureFindSecrets(t *testing.T) {
	...
	expectedResources := map[string]string{
		...
		"Web App":            fixture.Output("webapp_id"),
	...
	for label, identifier := range expectedResources {
		t.Run("detects secret in "+label, func(t *testing.T) {
			found := azureHasRiskForIdentifier(risks, identifier)
	...
	t.Run("all risks have non-empty impacted resource", func(t *testing.T) {
		for _, risk := range risks {
			assert.NotEmpty(t, risk.ImpactedARN, "ImpactedARN should not be empty for risk %s", risk.Name)
		}
	})
}

func azureHasRiskForIdentifier(risks []output.AurelianRisk, identifier string) bool {
	lowerID := strings.ToLower(identifier)
	for _, risk := range risks {
		if strings.Contains(strings.ToLower(risk.ImpactedARN), lowerID) {
			return true
		}
	}
	return false
