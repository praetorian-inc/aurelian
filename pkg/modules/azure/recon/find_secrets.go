...
type AzureFindSecretsConfig struct {
	...
	ScanMode  string `param:"scan-mode" desc:"Scan mode: critical (default) or all" default:"critical"`
}
...
func (m *AzureFindSecretsModule) Run(_ plugin.Config, out *pipeline.P[model.AurelianModel]) error {
	...
	subscriptionIDs, err := findSecretsResolveSubscriptionIDs(c.SubscriptionID, resolver)
	if err != nil {
		return err
	}
	...
	queries := pipeline.New[azuretypes.SubscriptionQuery]()
	pipeline.Pipe(resolvedSubs, func(sub azuretypes.SubscriptionInfo, qOut *pipeline.P[azuretypes.SubscriptionQuery]) error {
		qOut.Send(azuretypes.SubscriptionQuery{
			Subscription:  sub,
	...
	pipeline.Pipe(queries, lister.ListByTypes, listed)

	extractor := extraction.NewAzureExtractor(c.AzureCredential, c.ScanMode)
...
func findSecretsResolveSubscriptionIDs(ids []string, resolver *subscriptions.SubscriptionResolver) ([]string, error) {
...
func azureRiskFromScanResult(result secrets.SecretScanResult, out *pipeline.P[model.AurelianModel]) error {
	proof := azureBuildProofData(result, result.Match)
	...
	out.Send(output.AurelianRisk{
		Name:        azureFormatSecretRiskName(result.Match.RuleID),
...
func azureFormatSecretRiskName(ruleID string) string {
...
func azureBuildProofData(result secrets.SecretScanResult, match *types.Match) map[string]interface{} {
