...
// RegionsConfig deliberately does not embed AWSCommonRecon: that would add a
// regions parameter, making the module that answers "which regions are enabled"
// take the answer as input.
...
type AWSRegionsModule struct {
	...
	// enabledRegions is a test seam: helpers' region-lister interfaces and its
	// default config loader are unexported there, so a test in this package cannot
	// otherwise drive Run without live AWS credentials. Nil means "use the real
	// helper", which is how the instance registered in init() always runs.
...
// SupportedResourceTypes declares the Guard DISPATCH KEY for this module, not an
// enumeration claim: the module enumerates no resource types at all, it answers a
// question ABOUT the account. Returning nil would make the region coordinator
// undispatchable for EVERY target, not merely absent from listings.
...
// resolver is split out of Run so the default binding can be asserted without
// being invoked: reaching it through Run means a live AWS call, which on a
// credentialed machine can pass for the wrong reason.
