...
// Initialize prepares the formatter (implements plugin.Outputter).
func (a *FormatterAdapter) Initialize(cfg plugin.Config) error {
...
func (a *FormatterAdapter) formatRaw(val any) error {
	...
	// Write raw JSON directly to the writer, wrapped in an array to match Aurelian-style output
