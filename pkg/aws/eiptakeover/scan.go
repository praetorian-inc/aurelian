...
// Scan runs the full EIP dangling A record detection pipeline and calls emit
// for each dangling record discovered.
func Scan(opts ScanOptions, emit func(output.AurelianRisk)) error {
	...
	if err != nil {
		return fmt.Errorf("find a records: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("fetch allocated eips: %w", err)
	}
	...
	for _, rec := range aRecords {
		for _, ip := range rec.IPs {
			...
			count++
			emit(dangling.ToRisk())
		}
	}

	slog.Info("eip-takeover: scan complete", "dangling_count", count)
	return nil
}
