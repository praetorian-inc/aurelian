...
func (ga *GaadAnalyzerOld) AnalyzePrincipalPermissions() (*PermissionsSummary, error) {
	...
	ga.state.applyCreateThenUseEdges(summary)
...
func (ga *GaadAnalyzerOld) processUserPermissions(user types.UserDetail, evalChan chan<- *EvaluationRequest) {
	...
	for _, groupName := range user.GroupList {
		if group, exists := ga.getGroupByName(groupName); exists {
			...
			for _, attachedPolicy := range group.AttachedManagedPolicies {
				if policy := ga.state.getPolicyByArn(attachedPolicy.PolicyArn); policy != nil {
	...
	allActions := ga.state.ExtractActions(&identityStatements)
		...
		if isPrivEscAction(action) {
			...
			for _, resource := range ga.state.getResourcesByAction(Action(action)) {
				...
				if action == "sts:AssumeRole" {
					role := ga.state.RoleCache[resource.Arn.String()]
					if role != nil {
...
func (ga *GaadAnalyzerOld) processRolePermissions(role types.RoleDetail, evalChan chan<- *EvaluationRequest) {
	...
	for _, action := range allActions {
		if isPrivEscAction(action) {
			...
			resources := ga.state.getResourcesByAction(Action(action))
			if resources == nil {
			...
			for _, resource := range resources {
				...
				// Without the resource, the evaluator won't match the policy
				if action == "sts:AssumeRole" {
					if ga.state.RoleCache == nil {
						slog.Debug("Role cache is nil, skipping AssumeRole handling")
						continue
					}

					roleObj := ga.state.RoleCache[resource.Arn.String()]
					if roleObj == nil {
...
func (ga *GaadAnalyzerOld) processAssumeRolePolicies(evalChan chan<- *EvaluationRequest) {
	...
	for _, role := range ga.policyData.Gaad.RoleDetailList {
		...
		for _, stmt := range *role.AssumeRolePolicyDocument.Statement {
			...
			// Skip if types.Principal is nil
			if stmt.Principal == nil {
				slog.Debug(fmt.Sprintf("Skipping statement with nil types.Principal for role %s", role.Arn))
				continue
			}
			...
			for _, principal := range principals {
				...
				roleAccountID, tags := ga.state.getResourceDeets(role.Arn)
