...
// newResourceMap converts a slice of AWSResource to a cache.Map keyed by ARN (or ResourceID).
func newResourceMap(resources []output.AWSResource) cache.Map[output.AWSResource] {
	m := cache.NewMap[output.AWSResource]()
...
func TestGetResourceDetails_NotFoundFallsBackToARNParsing(t *testing.T) {
	state := NewAnalyzerMemoryState(types.NewAuthorizationAccountDetails("", nil, nil, nil, nil), orgpolicies.NewDefaultOrgPolicies(), cache.Map[output.AWSResource]{})
