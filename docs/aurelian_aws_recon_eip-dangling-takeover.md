## aurelian aws recon eip-dangling-takeover

Detects Route53 A records pointing to AWS IP addresses that are not allocated as Elastic IPs in this account. Such dangling records can be taken over by an attacker who allocates the released IP.

```
aurelian aws recon eip-dangling-takeover [flags]
```
...
```
  ...
  -h, --help                           help for eip-dangling-takeover
