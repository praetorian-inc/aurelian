...
* [aurelian aws recon amplify-config](aurelian_aws_recon_amplify-config.md)	 - Extracts AWS Amplify configuration from a web application by scanning HTML and JavaScript bundles for Cognito, AppSync, API Gateway, S3, and other AWS service details.
* [aurelian aws recon amplify-discover](aurelian_aws_recon_amplify-discover.md)	 - Discovers Amplify app branches by probing CloudFront distribution IDs against the <branch>.<distribution-id>.amplifyapp.com URL pattern. Amplify apps and their backing CloudFront distributions share the same distribution ID.
