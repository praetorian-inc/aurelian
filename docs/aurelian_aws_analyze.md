...
* [aurelian aws analyze graph](aurelian_aws_analyze_graph.md)	 - Analyzes a Neo4j graph populated by `aws recon graph --neo4j-uri` and surfaces privilege-escalation paths (non-admin principal → admin target) as risk findings. Requires --neo4j-uri pointing at the seeded graph.
